...
cat << EOF > /etc/sysctl.d/10-ipv4-forward.conf
##############################################################
# Enable IPv4 Forwarding
#
net.ipv4.ip_forward=1
EOF
sysctl -p /etc/sysctl.d/10-ipv4-forward.conf > /dev/null 2>&1
}

function disable_ipv6 {
cat << EOF > /etc/sysctl.d/90-ipv6-disable.conf
##############################################################
# Disable IPv6
#
...
EOF
sysctl -p /etc/sysctl.d/90-ipv6-disable.conf > /dev/null 2>&1
}
...
if [ $ENABLE_IPV4_FORWARD ]; then
	echo -n "Setting up IPv4 forwarding: "
	enable_ipv4_forward && echo "enabled" || echo "failed"
fi
...
deb-src http://mirror.yandex.ru/debian/ trixie main

deb http://mirror.yandex.ru/debian/ trixie-updates main
deb-src http://mirror.yandex.ru/debian/ trixie-updates main
